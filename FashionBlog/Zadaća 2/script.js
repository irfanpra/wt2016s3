...
window.onload = function () {
    ...
    for (var i = 0; i < datelabels.length; i++) {
		...
		if (i==0) d = new Date();
		if (i==1) d = new Date(new Date() - 400000);
		if (i==2) d = new Date(new Date() - 2*86500000);
        pomlabels[i].innerHTML = datelabels[i].innerHTML;
        if(ProtekloVrijeme(d) != -1) datelabels[i].innerHTML = ProtekloVrijeme(d);
	}
	var datelabels1 = document.getElementsByClassName("Datum1");
...
function timeSpan(datum) {
	...
	if(danas.getUTCMonth() == datum.getUTCMonth())
	{
		if((danas.getDay() >= datum.getDay() || danas.getDay() == 0) && (danas.getUTCDate() - datum.getUTCDate()) <= 7) 
...
function dropdownChanged() {
    ...
    var posts = document.getElementsByClassName("lijevo");
